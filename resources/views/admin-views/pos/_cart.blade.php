                    ...
                    @if(is_array($cartItem))
                        ...
                        <tr>
                            ...
                            <td class="align-items-center text-center">
                                <input type="number" data-key="{{$key}}" id="{{ $cartItem['id'] }}"
                                       class="amount--input form-control text-center"
                                       value="{{$cartItem['quantity']}}" min="1" max="{{ $product['total_stock'] }}"
...
<div class="box p-3">
    <dl class="row">
        ...
        <dd class="col-sm-6 text-right">
            ...
            - {{ Helpers::set_symbol($extra_discount) }}</dd>

        <dt class="col-sm-6">{{translate('tax')}} {{ \App\CentralLogics\Helpers::get_business_settings('product_vat_tax_status') === 'included'?  '(Included)': ''}}
            :
        </dt>
        ...
        <dd class="col-sm-6 text-right h4 b">{{ Helpers::set_symbol(round($total+$updated_total_tax+$delivery_charge, 2)) }}</dd>
    </dl>
    <div>
                    ...
                    <li style="display: {{ !session()->has('order_type') || session('order_type') == 'take_away' ?  'block' : 'none' }}">
                        <label>
                            <input type="radio" name="type" value="cash"
                                   hidden="" {{ !session()->has('order_type') || session('order_type') == 'take_away' ? 'checked' : '' }}>
                ...
                <form action="{{route('admin.pos.discount')}}" method="post" class="row">
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        <input type="number" min="0" max="" value="{{session()->get('cart')['extra_discount'] ?? 0}}"
                               id="extra_discount_input" class="form-control" name="discount" step="any"
