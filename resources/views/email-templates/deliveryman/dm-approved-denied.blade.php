...
<table style="width:100%;max-width:500px;margin:0 auto;text-align:center;background:#fff">
    ...
    <tr>
        <td style="padding:0 30px 30px; text-align:left">
            <span
                style="font-weight:500;display:block;margin: 20px 0 11px;">{{ translate('Hi') }} {{ $dm->f_name .' '. $dm->l_name }},</span>
            @if($dm->application_status == 'approved')
                <span
                    style="display:block;margin-bottom:14px">{{ translate('Your registration request has been approved by admin. Now you can login to GroFresh deliveryman app and view all the options') }}</span>
            @endif
            @if($dm->application_status == 'denied')
                <span
                    style="display:block;margin-bottom:14px">{{ translate('Your registration request has been denied by admin.') }}</span>
            @endif
            <br>
            <span class="border-top"></span>
            <span class="d-block"
                  style="margin-bottom:14px">{{ translate('Please contact us for any queries, we’re always happy to help.') }}</span>
            <span class="d-block">{{ translate('Thanks & Regards') }},</span>
            <span class="d-block" style="margin-bottom:20px"> {{$business_name}}</span>

            <img style="width:120px;display:block;margin:10px auto"
