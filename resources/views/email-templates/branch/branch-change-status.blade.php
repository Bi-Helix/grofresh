...
<head>
    ...
    <title>{{ translate('Branch Status Change') }}</title>
...
<table class="main-table">
    ...
    <tr>
        <td class="main-table-td">
            <img style="width:120px;display:block;margin:10px 0"
                 onerror="this.src='{{asset('/public/assets/admin/img//logo/main-logo.png')}}'"
                 src="{{ asset('storage/app/public/restaurant/' . $logo) }}" alt="public/img">
            @if($branch->status == 1)
                <h2>{{ translate('Your Branch Has Been Reopened By Admin') }}</h2>
                <div class="mb-1">{{ translate('Hi') }} {{ $branch->name?? '' }},</div>
                <div
                    class="mb-2">{{ translate('Admin has open your branch. you can login to panel and your branch will be shown in the customer app or website') }}</div>
                <img class="mb-2" src="{{asset('/public/assets/admin/img/email-templates/status-on.png')}}" alt="img">
            @endif
            @if($branch->status == 0)
                <h2>{{ translate('Your Branch Has Been Closed By Admin') }}</h2>
                <div class="mb-1">{{ translate('Hi') }} {{ $branch->name?? '' }},</div>
                <div
                    class="mb-2">{{ translate('Admin has closed your branch. you cannot login to panel and your branch will not be shown in the customer app or website') }}</div>
                <img class="mb-2" src="{{asset('/public/assets/admin/img/email-templates/status-off.png')}}" alt="img">
            @endif
            <hr>
            <div class="mb-2">
    ...
    <tr>
        <td>
                <span class="privacy">
                    <a href="{{ route('pages.privacy-policy') }}">{{translate('Privacy Policy')}}</a><a
                        href="{{ route('pages.about-us') }}">{{ translate('About Us') }}</a>
                </span>
