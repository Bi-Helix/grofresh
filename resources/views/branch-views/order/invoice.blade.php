...
@section('content')
    ...
    <div class="content container-fluid initial-38">
        <div class="row justify-content-center" id="printableArea">
            <div class="col-md-12">
                <center>
                    <input type="button" class="btn btn--primary non-printable text-white m-1"
                           onclick="printDiv('printableArea')"
                           value="{{translate('Proceed, If thermal printer is ready.')}}"/>
                    <a href="{{url()->previous()}}"
                       class="btn btn--danger non-printable text-white m-1">{{ translate('Back') }}</a>
                </center>
            ...
            <div class="initial-38-1">
                            ...
                            @if(isset($order->customer))
                                <h5>
                                    {{ translate('Customer Name') }} :<span
                                        class="font-light">{{$order->customer['f_name'].' '.$order->customer['l_name']}}</span>
                                </h5>
                                <h5>
                                    {{ translate('phone') }} :<span
                                        class="font-light">{{$order->customer['phone']}}</span>
                                </h5>
                                @php($address=\App\Model\CustomerAddress::find($order['delivery_address_id']))
                                <h5 class="text-break">
                                    {{ translate('address') }} :<span
                                        class="font-light">{{isset($address)?$address['address']:''}}</span>
                                </h5>
                            @endif
                        @else
                            @if($order->order_type == 'delivery')
                                @if(isset($order->delivery_address))
                                    <h5>
                                        {{ translate('Customer Name') }} :<span
                                            class="font-light">{{$order->delivery_address['contact_person_name']}}</span>
                                    </h5>
                                    <h5>
                                        {{ translate('Customer Name') }} :<span
                                            class="font-light">{{$order->delivery_address['contact_person_number']}}</span>
                                    </h5>
                                    <h5 class="text-break">
                                        {{ translate('address') }} :<span
                                            class="font-light">{{$order->delivery_address['address']}}</span>
                                    </h5>
                                @endif
                            @endif
                        @endif
                ...
                <div class="px-3">
                    <dl class="row text-right justify-content-center">
                        ...
                        <dt class="col-6">{{translate('Tax / VAT')}} {{ $vat_status == 'included' ? translate('(included)') : '' }}
                            :
                        </dt>
                    ...
                    <span class="initial-38-5">---------------------------------------------------------------------------------</span>
                    <span
                        class="d-block text-center">{{ $footer_text = \App\Model\BusinessSetting::where(['key' => 'footer_text'])->first()->value }}</span>
                </div>
            </div>
        </div>
    </div>
